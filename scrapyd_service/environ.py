...
class Environment(object):
    ...
    def __init__(self, config, initenv=os.environ):
        ...
        self.settings = dict(config.items('settings'))
        self.target_cfg_mapping = dict(config.items("cfg"))
    ...
    def get_environment(self, message, slot):
        ...
        if project in self.settings:
            project_cfg_path = self.target_cfg_mapping.get(project)
            project_path = str(Path(project_cfg_path).parent)
            env['SCRAPY_PROJECT_PATH'] = project_path
