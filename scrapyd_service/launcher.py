...
from .interfaces import IEnvironment, IPoller
from .persistence import FinishedSpidersDB
...
class Launcher(Service):
    ...
    def __init__(self, config, app):
        self.processes = {}
        self.finished = FinishedSpidersDB(config.get('dbs_dir', 'dbs'))
        self.dates_to_keep = config.getint('dates_to_keep', 1)
        self.max_proc = self._get_max_proc(config)
        self.runner = config.get("runner", 'scrapyd_service.runner')
        self.app = app
    ...
    def _process_finished(self, _, slot):
        ...
        self.finished.insert(process)
        if len(self.processes) == 0:
            # 当前无运行爬虫时候, 情况历史数据
            self.finished.delete(self.dates_to_keep)
        self._wait_for_project(slot)
...
class ScrapyProcessProtocol(protocol.ProcessProtocol):
    ...
    def processExited(self, data):
        log.msg(data, system="Launcher,%d/stderr" % self.pid)
